...
export default function CoinInfoSm({}: Props) {
    ...
    <Stack direction='row' spacing={1}>
      ...
      >
        ...
        {predictedSymbol?.predictedPrice !== undefined
          ? predictedSymbol.predictedPrice.toString().indexOf('.') !== -1 &&
            Number(predictedSymbol.predictedPrice.toString().indexOf('.')) >= 4
            ? Number(predictedSymbol.predictedPrice).toFixed(2)
            : Number(predictedSymbol.predictedPrice.toString().indexOf('.')) >=
              2
      ...
      <Typography
        ...
        component='div'
        className={`${
          Number(percentageDifference) > 0 ? 'text-LONG' : 'text-SHORT'
        }`}
        sx={{
          fontSize: {
            sm: 15,
